...
```{r}
  yer <- logistic_reg(mode = "classification") %>% 
    set_engine("glm") %>%
    fit(dvote ~ gender + party + state + office, data = clean) %>%
  predict(new_data = clean) %>% bind_cols(clean %>% select(dvote)) %>%
...
set.seed(10)
train_test_split <-
  initial_split(data = clean, prop = 0.01) 

train_tbl <- train_test_split %>% training() 
test_tbl  <- train_test_split %>% testing()


  yert2 <- logistic_reg(mode = "classification") %>% 
    set_engine("glm") %>%
    fit(dvote ~ gender + party + state + office, data = train_tbl) %>%
  predict(new_data = train_tbl) %>% bind_cols(train_tbl %>% select(dvote)) %>%
  metrics(truth = dvote, estimate = .pred_class) %>%
  gt() 
  
  yert3 <- logistic_reg(mode = "classification") %>% 
    set_engine("glm") %>%
    fit(dvote ~ gender + party + state + office, data = train_tbl) %>%
  predict(new_data = test_tbl) %>% bind_cols(test_tbl %>% select(dvote)) %>%
  metrics(truth = dvote, estimate = .pred_class) %>%
  gt() 
